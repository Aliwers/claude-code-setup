...
Скрипт кладёт бэкап текущего `~/.claude` в `~/.claude/backups/`, показывает, что
перезапишет, и спрашивает подтверждение. Файлов вне `~/.claude` не трогает —
проекты и их локальные `.claude/` в безопасности. Идемпотентен.
